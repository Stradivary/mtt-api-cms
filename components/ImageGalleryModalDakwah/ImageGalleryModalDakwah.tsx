...
export default function ImageGalleryModalDakwah({
  open,
  onClose,
  onSelectImage,
  ...
  const fetchImages = async (reset = false) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/galery-dakwah?page=${reset ? 1 : page}&limit=${limit}`);
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch('/api/galery-dakwah', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDelete = async (img: GalleryItem) => {
    ...
    try {
      const res = await fetch(`/api/galery-dakwah/${img.id}`, {
        method: 'DELETE',
      });
      ...
      <DialogContent className="max-w-3xl left-[calc(50%+170px)] -translate-x-1/2">
        ...
        <Tabs value={tab} onValueChange={setTab} className="mt-4">
          <TabsList className="w-full grid grid-cols-2 mb-4">
            <TabsTrigger value="gallery" className="cursor-pointer">Galeri</TabsTrigger>
          ...
          <TabsContent value="upload">
            <div className="flex flex-col items-center justify-center gap-4 mt-6">
              ...
              <p className="text-sm text-muted-foreground">
                Maksimal ukuran gambar 200KB (folder: <code>dakwah</code>)
              </p>
            </div>
          </TabsContent>
        </Tabs>
